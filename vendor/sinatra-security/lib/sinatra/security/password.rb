  ...
  module Security
    ...
    module Password
      def self.included(model_class)
        model = model_class.user_adaptor

        model.attribute :crypted_password
        model_class.send :attr_accessor, :password, :password_confirmation


        model.before_save do
          if !self.password.to_s.empty?
            model.write_local self, :crypted_password, Hashing.encrypt(password)
          end
          super()
        end
      end
